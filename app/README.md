...
- `generation.py` — calls an OpenAI chat model with a strict system prompt:
  answer only from the retrieved passages, cite `[Source Title — Section
  Title]` inline for every claim, and refuse rather than guess when the
  context doesn't support a confident answer. This is the v1 baseline to
  evaluate retrieval quality against (see `eval/`) — not yet optimized.
...
`OPENAI_API_KEY` set to answer questions.
